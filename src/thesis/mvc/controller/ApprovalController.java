...
public class ApprovalController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (new OrderImplement().getOrderById(orderID).getOrderStatus().equalsIgnoreCase("CANCELLED")) {
	        ...
	        audit.setActionTaken("order ID: " + orderID + " was rejected by Pharmacist ID No:" + session.getAttribute("Pharmacist"));
			session.setAttribute( "Message" , "Order Has been Rejected by the user" );
	        ...
	        audit.setActionTaken("order ID: " + orderID + " was approved by Pharmacist ID No:" + session.getAttribute("Pharmacist"));
			session.setAttribute( "Message" , "Order Has been Approved by the user" );
		} else if (action.equalsIgnoreCase("OrderInvalidate")){
			Order order = new OrderImplement().getOrderById(orderID);
			if (new ShopAction().RefundOrder(order, "Pharmacist Invalidated the order")) {
		        audit.setActionTaken("order ID:" + orderID + " was invalidated and cancelled by Pharmacist ID No:" + session.getAttribute("Pharmacist"));
			} else {
		        audit.setActionTaken("order ID:" + orderID + " was invalidated and cancelled by Pharmacist ID No:" + session.getAttribute("Pharmacist") + " But Failed to refund due to error");
			}
		   	approvalAction.pharmacistApproval( orderID, 3 );
			session.setAttribute( "Message" , "Order Has been invalidated by the user" );
			...
			session.setAttribute( "Message" , "Return order Has been approved by the user" );
	        audit.setActionTaken("order ID: " + orderID + " was approved for returns by Pharmacist ID No:" + session.getAttribute("Pharmacist"));
		} else if (action.equalsIgnoreCase("ReturnApprove")){
		   	approvalAction.pharmacistApproval( orderID, 5 );
			session.setAttribute( "Message" , "Return order Has been rejected by the user" );
	        audit.setActionTaken("order ID: " + orderID + " was disapproved for returns by Pharmacist ID No:" + session.getAttribute("Pharmacist"));
		}
